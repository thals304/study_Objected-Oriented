	...
	void loginMenu() {
		...
		while (true) {
			...
			if (selectMenu == 1) {
				...
				// 구현한게 정답이 맞나? 
				// FileManager.getInstance().save();는 언제마다 해줘야 하는가? 회원 & 계좌 추가, 삭제 등에서
				...
				if (UserManager.getInstance().userList[identifier].accCnt == 1) {
					...
					UserManager.getInstance().userList[identifier].acc = null; // UserManager.getInstance().userList[identifier] = null;이 아닌 .acc가 null이 되도록 해야함
					FileManager.getInstance().save(); // 삭제됐을 때  파일도 수정 및 저장
				}
				else {
